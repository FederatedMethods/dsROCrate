...
Here we will add details for the safe outputs, currently only log files from
the operations executed by the user within a specific period. Set the period
using `logs_from` and `logs_to`. Here we will be using the function 
`dsROCrate::safe_output`.

**⚠️NOTE:** Similar to `dsROCrate::safe_setting`, the `dsROCrate::safe_output` 
function requires of administrator rights, so here, we will have to login with 
